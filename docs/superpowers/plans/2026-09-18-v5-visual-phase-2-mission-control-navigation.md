...
- Neo pointer effects run only on `(hover: hover) and (pointer: fine)` devices and are disabled under reduced motion.
- Magnetic movement is visual-only, clamped to 2px in either axis, and never changes layout or hit targets.
- Pointer tracking must write CSS custom properties through requestAnimationFrame/ref-managed DOM updates; no React state update is allowed per pointer frame.
- Do not add scan lines, particle fields, cursor trails, heavy 3D tilt, continuous blur animation, or continuously animated box shadows.
- Reduced motion uses short fades/state switches and retains complete keyboard/focus behavior.
...
- `components/ui/MagneticAction.jsx` — fine-pointer-only visual translation wrapper, clamped to 2px.
- `components/ui/PointerSpotlightSurface.jsx` — rAF/CSS-variable pointer spotlight wrapper with reduced-motion/coarse-pointer fallback.
...
- `tests/neo-mission-control-ui.test.mjs`
...
## Task 2: Add Mission Control Flag, Neo Interaction Primitives, Library Request Seam, and Complete Home Surface
...
- `<PointerSpotlightSurface className children />` owns a DOM ref, matches `(hover: hover) and (pointer: fine)`, writes `--spotlight-x`/`--spotlight-y` through a single pending requestAnimationFrame, and clears the pending frame on unmount. It does not store pointer coordinates in React state.
- `<MagneticAction disabled children className />` keeps the hit target stationary while translating only an inner visual wrapper; it clamps visual translation to `[-2, 2]` px per axis and returns to `{x:0,y:0}` on leave/blur. It is disabled for coarse pointers and reduced motion.
...
- [ ] **Step 2: Add RED UI contracts** in `tests/mission-control-ui-contract.test.mjs` and `tests/neo-mission-control-ui.test.mjs`. Assert the Mission Control component includes `Search prompts, commands, workflows`, `Continue Working`, `Featured Prompt Packs`, `Prompt Health`, `Activity`, and `Smart Collections`; telemetry/cloud sections are conditional; `app/page.jsx` has `libraryRequest`; `PromptLibraryV5.jsx` accepts `externalRequest` plus `onExternalRequestHandled`; Neo primitives include `(hover: hover) and (pointer: fine)`, `requestAnimationFrame`, `--spotlight-x`, `--spotlight-y`, a 2px clamp, and no `setState(` in pointer handlers.

```js
test('neo pointer effects are bounded and DOM-driven', () => {
  const spotlight = fs.readFileSync('components/ui/PointerSpotlightSurface.jsx', 'utf8');
  const magnetic = fs.readFileSync('components/ui/MagneticAction.jsx', 'utf8');
  ...
  assert.match(magnetic, /Math\.max\(-2, Math\.min\(2,/);
});
```

- [ ] **Step 3: Run RED.** `node --test tests/feature-flags.test.mjs tests/mission-control-ui-contract.test.mjs tests/neo-mission-control-ui.test.mjs` and confirm failures are missing flag/components/contracts rather than unrelated regressions.
- [ ] **Step 4: Add `V5_MISSION_CONTROL`** to `lib/ui/feature-flags.mjs` and environment mapping.
- [ ] **Step 5: Implement `PointerSpotlightSurface.jsx` and `MagneticAction.jsx`.** Use refs and `window.matchMedia`; add/remove media listeners in effects; use CSS variables/transforms; honor `useReducedMotion`; do not move the outer hit target.
- [ ] **Step 6: Add Neo surface CSS** in `app/globals.css`: spotlight radial gradient reads `--spotlight-x/y`, cyan as primary local light and purple as secondary depth accent, no continuously animated shadow/blur, and `@media (hover: none), (pointer: coarse)` disables pointer-only decoration.
- [ ] **Step 7: Implement the one-shot Library request seam.** In `app/page.jsx`, add helpers that set `{ type:'prompt', id }` / `{ type:'view', viewId }` then navigate to `library`; clear only from `onExternalRequestHandled`. In `PromptLibraryV5`, set `hydrated=true` after loading local data, then consume a valid request in an effect. If a hydrated prompt request cannot be found, clear it without opening arbitrary content.
- [ ] **Step 8: Implement `MissionControl.jsx`.** Use `GlassSurface`, `GlassGlyph`, `MetricChip`, `AnimatedNumber`, `PointerSpotlightSurface`, `MagneticAction`, and Motion layout primitives. Wrap only the hero command surface in pointer spotlight behavior. Apply magnetic motion to the hero primary command CTA only. Render resolved default packs as showcase cards with CSS/SVG micrographics; their hover/focus animation runs once on interaction and does not continuously loop. Hide every empty/disabled section.
- [ ] **Step 9: Wire Home in `app/page.jsx`.** When `V5_MISSION_CONTROL` is true and `activePage==='home'`, render Mission Control. Initial page is `home` only when the flag is true; otherwise retain current `library` initial behavior. Pass `healthEnabled` from `V5_PROMPT_HEALTH`, `smartCollectionsEnabled` from `V5_SMART_COLLECTIONS`, and `usageEnabled` from `V5_USAGE_ANALYTICS`. Pass `cloudStatus=null` until a real cloud-sync state seam is available.
- [ ] **Step 10: Run GREEN.** `node --test tests/feature-flags.test.mjs tests/mission-control.test.mjs tests/mission-control-ui-contract.test.mjs tests/neo-mission-control-ui.test.mjs tests/v5-core-ui-contract.test.mjs`.
- [ ] **Step 11: Commit.** `git add lib/ui/feature-flags.mjs tests/feature-flags.test.mjs components/home/MissionControl.jsx components/ui/PointerSpotlightSurface.jsx components/ui/MagneticAction.jsx components/prompt/PromptLibraryV5.jsx app/page.jsx app/globals.css tests/mission-control-ui-contract.test.mjs tests/neo-mission-control-ui.test.mjs && git commit -m "feat: add neo mission control home"`.

## Task 3: Build Command Items and Neo Command Palette V5
...
- The palette focus surface uses `PointerSpotlightSurface` only on fine-pointer devices; keyboard navigation must produce the same selected-row hierarchy without requiring pointer movement.
- The selected command row may use `MagneticAction` only on its primary execution affordance; list rows themselves must not shift under the pointer.
- `openCommandPalette()` in `app/page.jsx` performs a fresh `loadPromptCatalogState(window.localStorage, AI_PROMPT_LIBRARY)` read at open time, then builds items.
...
- [ ] **Step 2: Add RED component contract** `tests/command-palette-v5-ui.test.mjs` asserting `role="dialog"`, `aria-modal`, ArrowDown/ArrowUp/Enter/Escape handling, `AnimatePresence`, fresh `loadPromptCatalogState` use when opening, prompt actions routed through the Library request seam, `PointerSpotlightSurface` use around the focus panel, and no animation-delayed input focus.
...
- [ ] **Step 5: Implement `CommandPaletteV5.jsx`.** On open, save `document.activeElement`, focus the search input synchronously/effect-on-open without waiting for animation, rank with the existing model, clamp selection after result changes, execute selected item on Enter, close on Escape, and restore prior focus on close. Use Motion spring on desktop and a full-screen sheet breakpoint on mobile. Add a subtle cyan/purple spotlight behind the focus glass; selected rows use one shared visual selection state for keyboard and pointer input.
...
- [ ] **Step 7: Run GREEN.** `node --test tests/command-palette.test.mjs tests/command-palette-v5-ui.test.mjs tests/mission-control-ui-contract.test.mjs tests/neo-mission-control-ui.test.mjs`.
- [ ] **Step 8: Commit.** `git add lib/ui/command-items.mjs components/command/CommandPaletteV5.jsx app/page.jsx app/globals.css tests/command-palette.test.mjs tests/command-palette-v5-ui.test.mjs tests/neo-mission-control-ui.test.mjs && git commit -m "feat: add neo command palette v5"`.

## Task 4: Morphing Navigation, Stable Page Motion, Mobile Dock, and Toast Feedback
...
- Sidebar uses exactly one shared Motion active indicator with `layoutId="v5-nav-active"`; expanded state is a soft glass fill + cyan rail, collapsed state morphs to an icon-sized capsule rather than remounting a second indicator.
...
- `MobileDock({ activePage, onNavigate, onNewPrompt, onOpenMore })` presents Library, Workspaces, Create, Activity (routes to analytics), More and uses one shared active indicator with `layoutId="v5-mobile-active"`.
...
- Reduced motion changes page/sidebar/dock movement to short fade/state switches, not slower springs.
...
- [ ] **Step 1: Add RED navigation/page-transition contracts** in `tests/navigation-motion-ui.test.mjs` for collapsed/expanded Sidebar props, the single shared `layoutId="v5-nav-active"`, `layoutId="v5-mobile-active"`, safe-area mobile dock CSS, disabled create actions when callbacks are absent, persistent AppShell outside `PageTransition`, transition values 10px / 0.20s / 0.12s, reduced-motion fallback, and absence of `mode="wait"`.
...
- [ ] **Step 5: Update `Sidebar.jsx`.** Replace the current dot-only active state with one shared active rail/background element using `layoutId="v5-nav-active"`; animate width via Motion spring while keeping icons in a fixed-width cell. Reuse the same active element in expanded/collapsed states; do not render separate active decorations that blink between states.
...
- [ ] **Step 7: Update `MobileDock.jsx` and add `CreateActionSheet.jsx`.** Respect `env(safe-area-inset-bottom)`, use `layoutId="v5-mobile-active"`, no hover dependency, at least 44px tap targets, and a central Create control that may sit visually above the dock without covering content.
...
- [ ] **Step 10: Add navigation Neo CSS** in `app/globals.css`: cyan active rail, restrained purple depth, stable icon cell, mobile dock glass depth, no scanline/glitch decoration, and coarse/reduced-motion fallbacks.
- [ ] **Step 11: Run GREEN.** `node --test tests/navigation-motion-ui.test.mjs tests/toast-queue.test.mjs tests/feature-flag-integration.test.mjs tests/neo-mission-control-ui.test.mjs`.
- [ ] **Step 12: Commit.** `git add components/shell components/ui/ToastViewport.jsx lib/ui/v5-navigation.mjs lib/ui/toast-queue.mjs app/globals.css tests/navigation-motion-ui.test.mjs tests/toast-queue.test.mjs tests/neo-mission-control-ui.test.mjs && git commit -m "feat: upgrade neo navigation and feedback motion"`.

## Task 5: Localization, Accessibility, Performance Contracts, and Phase Verification
...
- [ ] **Step 3: Extend `tests/neo-mission-control-ui.test.mjs`** to assert pointer-only effects use fine-pointer media queries, use rAF/CSS variables rather than React pointer state, magnetic clamp is 2px, reduced-motion disables magnetic/spotlight motion, no `scanline|cursor-trail|particle-field|rotateX|rotateY` markers appear in the new Phase 2 components, and Mission Control metrics are still conditionally rendered from real data.
- [ ] **Step 4: Run targeted accessibility/model tests.** `node --test tests/i18n-coverage.test.mjs tests/command-palette-v5-ui.test.mjs tests/navigation-motion-ui.test.mjs tests/toast-queue.test.mjs tests/mission-control-ui-contract.test.mjs tests/neo-mission-control-ui.test.mjs`.
- [ ] **Step 5: Run full suite.** `npm test`; confirm 80-prompt regression, Pack reference semantics, Prompt Health, Legacy fallback, feature-flag defaults, and reduced-motion contracts remain green.
- [ ] **Step 6: Build.** `npm run build` and verify `.open-next/worker.js`, `.open-next/assets`, `.open-next/.build/open-next.config.edge.mjs`.
- [ ] **Step 7: Cloudflare dry-run.** `npx wrangler deploy --dry-run --outdir .wrangler-dry-run`.
- [ ] **Step 8: Scope audit.** Run `git diff --name-only "$(git merge-base main HEAD)"...HEAD`; confirm no Supabase migration, provider implementation, prompt catalog content, production secrets, or parallel Neo route tree changed.
- [ ] **Step 9: Open Phase 2 PR and stop before merge.** Record exact verification results and require separate explicit `merge` authorization. Do not deploy production.