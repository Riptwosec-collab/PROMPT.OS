...
export function createRunSessionController({
  ...
  const finish = (context, status, { error = null } = {}) => {
    if (!context) return Promise.resolve(clone(snapshot.session));
    if (context.terminalPromise) return context.terminalPromise;

    context.finalized = true;
    stopHeartbeat(context);
    context.terminalPromise = (async () => {
      const at = now();
      const finalPayload = {
        status,
  ...
  async function stop() {
    ...
    if (!context) return clone(snapshot.session);
    if (context.terminalPromise) return context.terminalPromise;
