...
const THAI_TEXT = Object.freeze({
  ...
  VERSION_PERFORMANCE: 'ประสิทธิภาพแต่ละเวอร์ชัน',
  NO_DATA: 'ไม่มีข้อมูล',
  ...
  COMPARE: 'เปรียบเทียบ',
  'No note': 'ไม่มีบันทึก',
  'Initial imported version': 'เวอร์ชันเริ่มต้นจากการนำเข้า',
  'Edited from prompt modal': 'แก้ไขจากหน้าต่างพรอมต์',
  'Initial version': 'เวอร์ชันเริ่มต้น',
  'Compiled from Prompt Builder': 'สร้างจากตัวสร้างพรอมต์',

  RUN_HISTORY: 'ประวัติการรัน',
  NO_RUNS: 'ยังไม่มีการรัน',
  HISTORY: 'ประวัติ',
  META: 'ข้อมูลรัน',
  ...
  Local: 'ในเครื่อง',
  Manual: 'กำหนดเอง',
  'Manual Result': 'ผลลัพธ์ที่ใส่เอง',
...
const THAI_PATTERNS = Object.freeze([
  ...
  [/^RES (\d+)$/, (_, count) => `ผลลัพธ์ ${count}`],
  [/^(\d+)R \/ (\d+)C$/, (_, runs, copies) => `${runs} รัน / ${copies} คัดลอก`],
  ...
  [/^(\d+) runs · (\d+) excellent$/, (_, runs, excellent) => `${runs} รัน · ยอดเยี่ยม ${excellent} ครั้ง`],
  [/^Restored from v(.+)$/, (_, version) => `กู้คืนจาก v${version}`],
  [/^PUSH\.\.\.$/, () => 'กำลังอัปโหลด...'],
  [/^PULL\.\.\.$/, () => 'กำลังดาวน์โหลด...'],
  ...
  [/^PUSH_FAILED$/, () => 'อัปโหลดขึ้นคลาวด์ไม่สำเร็จ'],
  [/^PULL_FAILED$/, () => 'ดึงข้อมูลจากคลาวด์ไม่สำเร็จ'],
  [/^MAGIC_LINK_FAILED$/, () => 'ส่งลิงก์เข้าสู่ระบบไม่สำเร็จ'],
