...
export function createWorkspace(input = {}) {
  return normalizeWorkspace(input, Number.isFinite(Number(input.order)) ? Number(input.order) : 0);
}

export function createFolder(input = {}) {
  return normalizeFolder(input, Number.isFinite(Number(input.order)) ? Number(input.order) : 0);
}
...
export function movePromptToFolder(prompts = [], promptId, { workspaceId = 'personal', folderId = null } = {}) {
  return movePrompt(prompts, promptId, workspaceId, folderId);
}

export function removePromptFromFolder(prompts = [], promptId) {
  return prompts.map((prompt) => (
    String(prompt.id) === String(promptId)
