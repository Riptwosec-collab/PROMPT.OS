...
import { CORE_PROMPT_QUALITY_V2_METADATA } from './catalog/quality-v2/metadata-core.mjs';
import { RESEARCHED_PROMPT_QUALITY_V2_METADATA_A } from './catalog/quality-v2/metadata-researched-a.mjs';
import { RESEARCHED_PROMPT_QUALITY_V2_METADATA_B } from './catalog/quality-v2/metadata-researched-b.mjs';
import { QUALITY_V2_NEW_PROMPT_SPECS, QUALITY_V2_NEW_PROMPT_METADATA } from './catalog/quality-v2/new-prompts.mjs';
import { enrichBuiltInCatalog } from './catalog/quality-v2/index.mjs';
...
const UPDATED_AT = '2026-09-25T04:40:00.000Z';
...
const TEXTAREA_VARIABLES = new Set([
  'content', 'article', 'paper', 'code', 'transcript', 'sources', 'schema', 'requirements',
  'symptoms', 'routing_output', 'path_evidence', 'show_output', 'logs', 'environment', 'traffic_flow',
  'acl_or_rules', 'alert', 'evidence', 'email_content', 'headers', 'context', 'pipeline', 'recent_changes',
  'change_summary', 'test_evidence', 'deployment_plan', 'rollback_plan', 'expected_config', 'observed_config',
  'lock_evidence', 'transaction_context', 'data_description', 'sample_data', 'goal', 'funnel_steps',
  'segment_data', 'decision_or_claim', 'criteria', 'options', 'constraints', 'time_context', 'preferences',
  'research_goal', 'current_evidence', 'exam_objectives',
]);
...
function buildPrompt(spec) {
  ...
  const displayTitleTh = translateComplete('th', spec.displayTitle);
  const descriptionTh = translateComplete('th', spec.description);
...
const CORE_PROMPTS = enrichBuiltInCatalog(
  PROMPT_SPECS.map(buildPrompt),
  CORE_PROMPT_QUALITY_V2_METADATA,
);

const RESEARCHED_PROMPT_QUALITY_V2_METADATA = Object.freeze({
  ...RESEARCHED_PROMPT_QUALITY_V2_METADATA_A,
  ...RESEARCHED_PROMPT_QUALITY_V2_METADATA_B,
});

const RESEARCHED_PROMPTS_V2 = enrichBuiltInCatalog(
  RESEARCHED_PROMPT_LIBRARY,
  RESEARCHED_PROMPT_QUALITY_V2_METADATA,
);

const NEW_PROMPTS_V2 = enrichBuiltInCatalog(
  QUALITY_V2_NEW_PROMPT_SPECS.map(buildPrompt),
  QUALITY_V2_NEW_PROMPT_METADATA,
);

export const AI_PROMPT_LIBRARY = Object.freeze([
  ...CORE_PROMPTS,
  ...RESEARCHED_PROMPTS_V2,
  ...NEW_PROMPTS_V2,
].map((prompt) => Object.freeze({ ...prompt, catalogVersion: PROMPT_CATALOG_VERSION })));
