...
export function validateBuiltInPrompt(prompt = {}) {
  ...
  const inputGuide = prompt.inputGuideTh && typeof prompt.inputGuideTh === 'object' ? prompt.inputGuideTh : {};
  for (const [name, field = {}] of Object.entries(config)) {
    if (field.required) {
      ...
      if (!text(field.helpTh) && !text(inputGuide[name])) addError(errors, 'missing-variable-help-th', `Missing Thai help for ${name}`, `variableConfig.${name}.helpTh`);
    }
